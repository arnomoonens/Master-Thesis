...
As was stated, first the set of source tasks needs to be selected. Again, this can be done by a human in case of a human-guided scenario. However, the selection may also be done by the agent itself. It can select the source tasks that lead to to highest performance for the target task. The agent may also just aim to avoid negative transfer, which means that the specific selection of source tasks leads to worse learning performance for the target task.\\

Instead of just knowing that tasks are related, many methods also need to know how tasks are related, using a task mappings. This is necessary to make the knowledge gained on the source tasks useful for the target task. Tasks may differ in state and action labels and the in the semantic meaning of these labels. In the mountain car environment, one can have a task where the goal is on the opposite side. As such, taking the action \texttt{Left} has a different meaning for these tasks. Again, these mappings can be provided by a human or they can be learned by the agent.\\
...
\subsection{Metrics}
To evaluate transfer learning algorithms, several metrics exist.
