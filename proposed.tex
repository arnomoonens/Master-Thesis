...
\Repeat{$H > H_{max}$}{
    ...
    $h_{start} \gets h$\;
    Initialize state $s_h$\;
    \Repeat{terminal $s_h$ \textbf{or} $h-h_{start} = h_{max}$}{
        Perform $a_h$ according to policy $\pi(a_h\vert s_h;\theta^{(t)})$\;
        Receive reward $r_h$ and new state $s_{h+1}$\;
        $h \gets h + 1$\;
        }
    \For {$i \in \{h-1,\dots,h_{start} \}$} {
        Accumulate gradients w.r.t. $\theta^{(t)}$: $d\theta^{(t)} \gets d\theta ^{(t)}+ \alpha \nabla_{\theta^{(t)}} \log \pi_{\theta^{(t)}}(s_i,a_i)v_i$\;
    }
...
\Repeat{$H > H_{max}$}{
    ...
    Initialize state $s_h$\;
    \Repeat{terminal $s_h$ \textbf{or} $h-h_{start} = h_{max}$}{
        Perform $a_h$ according to policy $\pi(a_h \vert s_h; L  s^{(t)})$\;
        Receive reward $r_h$ and new state $s_{h+1}$\;
        $h \gets h + 1$\;
        }
    \For {$i \in \{h-1,\dots,h_{start} \}$} {
