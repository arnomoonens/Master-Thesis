...
To help with this, reinforcement learning algorithms often use a state-value function $V(s)$.
...
\end{equation}
There, we treat the terminal state as a state that always transitions to itself with zero reward. As such, treating an episodic task the same way as a continuing task doesn't influence the result, even if we discount the returns. All $n$-step returns that last up to or past termination will have the same value as the complete return.\\
...
To combine TD($\lambda$) and Sarsa, called Sarsa($\lambda$), we need eligibility traces for each state-action pair: $e_t(s,a)$. As such, the updates become:
\begin{equation}
...
When we are sure about the estimate of the state $s_t$, it can be set to zero so we don't have to use estimates of the following states anymore.
...
Policy gradient methods aim to parametrize a policy instead of storing a value for each state-action pair
...
Here, it is again possible to use function approximation.
